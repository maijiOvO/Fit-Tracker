...
import { getDataEnv, isDemo, isOffline, statePath } from './appEnv';
 ...
 *
 * 演示构建（demo）与单机发行版（solo）里置空 —— 理由就是上面那条「公网可直达」：
 * 这两种产物都是交到别人手上的，把主机名留在里面等于把个人数据服务器的公网地址
 * 递给每一个拿到包的人，只剩一把 key 挡着。而发行版 APK 比 demo 更糟：
 * key 也会被一起烙进去，解压 assets/public 就能读到。
 *
 * 这里直接查 import.meta.env 而不复用 appEnv 的 isDemo() / isSolo()，是刻意的：
 * vite 把这两个变量替换成字面量，三元于是在压缩期折叠、另一支的
 * 字符串被整个丢掉。改成跨模块的函数调用则折不掉 —— 运行时行为一样，
 * 但主机名会原样留在 bundle 里，这层防护当场归零。
 *
 * ⚠️ **改动前后都要 grep 产物**，这是唯一能证明它真的被折掉的手段：
 *    npm run build:solo && grep -r "taild995c6" dist/   ← 必须零命中
 */
export const DEFAULT_API_BASE_URL =
  import.meta.env.VITE_FITLOG_DEMO === 'true' || import.meta.env.VITE_FITLOG_SOLO === 'true'
    ? ''
    : 'https://hometj.taild995c6.ts.net';
...
export function isRemoteConfigured(): boolean {
  // 断网构建（demo / solo）永不联网：这里返回 false，调度器 / pull / push / 同步按钮
...
async function remoteFetch(
  ...
  // 🔒 硬守卫：断网构建（demo / solo）一个请求都不许发出去。
  //    isRemoteConfigured() 已经让所有调用点 no-op，这里是第二道 ——
  //    前者是约定（新增调用点可能忘了查），后者是保证（唯一出口拦得死）。
  //    第三道在原生层：solo 变体的 AndroidManifest 直接删掉 INTERNET 权限，
  //    就算前两道都被绕过，系统也不会放行 —— 见 android/app/src/solo/。
  if (isOffline()) {
    throw new RemoteError(
      `[fitlog] blocked: offline build attempted ${method} ${path}`,
      'env-guard',
    );
  }
