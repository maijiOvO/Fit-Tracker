...
点按语义不变；**长按 FAB 120ms 后，六枚部位印从拇指下扇形摊开**，
滑到某枚松手＝落章开练——标题填好、动作弹层弹出，一笔完成原来的两屏两击。
隐喻仍是「从印谱里挑一枚印」，只是印谱递到了拇指底下；
Pinterest 长按辐射菜单的骨架，加 press-hold-slide 的连贯性。
交互 demo（含调参面板）：`docs/demos/fab-seal-fan.html`。

**几何**（初值定在 demo 里，待真机调）：
以 FAB 圆心为原点，半径 **175px**（160 时印与印只剩 1px 缝），
从 95° 扫到 10°、跨度 85°；印面 **46×46**（比页内印谱的 52 略小，姿态仍是朱文正置）；
命中判定＝最近印中心距指尖 **≤48px**（不是命中印面本身——滑选手势的目标要虚胖）。
顺序固定为印谱阅读序（胸→制），**不按使用频率动态排**：位置稳定才长得出肌肉记忆。

**状态机**：

- `pressing`：前 120ms 什么都不出（同 §5.3 长按进度的规则，点按不许闪印谱）；
- `open`：stagger 32ms 摊开，悬停印渗墨、沿径向外浮 6px（别压在指头底下）、浮出部位名标签；
- 松手在印上＝提交（盖章过冲 + seal 震动）；
- 松手总时长 **<250ms** ＝仍按点按算，走原路径；
- ≥250ms 原地松手＝**留在点选模式**（点印可选、点空白收回）——比 Pinterest 的
  「松手即取消」宽容，第一次没滑到位不算白按。
- 「制」分支与页内一致：不填名、聚焦标题、弹层不弹。

震动三处：开扇 14ms、换悬停 12ms、提交 seal 波形，全走 `haptic()` 的 55ms 去抖。
快速点按时在 FAB 旁闪「按住可展开印谱」（LongPressAffordance 的 hint 思路），
页内印谱底部放同一句小技巧——手势是加速器，可见路径永远在。

⚠️ 三个实现坑（demo 里都踩过一遍）：

1. **`pointercancel` 必须完整复位**（清 timer、收扇、清悬停）——同 §12.3 粘滞布尔的教训，
   来电或系统手势打断后不能留下半开的扇面或吃掉下一次点按。
2. **stagger 用 setTimeout 加类，不用行内 `transition-delay`**——后者会赖在行内样式上，
   把之后悬停态的 120ms 过渡也拖慢 160ms。
3. 点选模式给印章开的 `pointer-events:auto` **收扇时必须还原**，
   否则六枚隐形印会挡住底下页面的点击。

### 12.5 操作层通则（12.4–12.9 共同遵守）

1. **手势是加速器，不是唯一入口。** 每个手势都有可见的点按路径做后备
   （印谱扇开 ↔ 页内印谱；翻月横滑 ↔ ‹ › 按钮），并自带一次自教学
   （短按闪标签 / 页内小技巧文案）。
2. **一个手势词全站只有一个意思。** 长按＝次级操作（子组 / 删组 / 管理菜单 /
   拖动排序 / 时间线菜单），横向拖＝改值或翻页，点按永远是主动作。
   同一元素上叠第二个长按语义前先看 §12.2 的「打架」论证。
3. **破坏性操作用「先执行 + 撤销」替代确认弹窗**（手势时代的标准解法：
   手势错误率天然高于点按，弹窗挡不住手滑、只挡得住流畅）。
   撤销窗口期内不写 tombstone —— app 被杀的失败模式是「没删成」而不是「误删了」。
   既有入口（Dashboard 按钮等）保留确认弹窗不动。
4. 实现三件套（每个手势都要过一遍）：`pointercancel` 完整复位（§12.3 粘滞布尔）、
   动画收尾回调幂等（transitionend + 兜底 timer 只许进一个）、
   拖完的 click 用时间戳吞掉。

### 12.6 底稿预填（上次数据）

添加练过的动作时，上次同动作的**每一组**以 `SetLog.ghost` 底稿行躺进来：
淡墨 + 朱砂虚线（§5.3 里「这是抄的，等你确认」那套语言的跨训练推广），
组号是一枚虚线「未刻的印」。出处写在眉批：「底稿 · 上次 X月X日」。

- **点组号＝照抄描实**（一组一击，效率的主要来源）；
- **改哪格，记哪格**：打字 / scrub / 点竭 / 加子组，任何编辑都让整行转正，
  转正播文字版墨色过冲（`ink-text`，accent-ink → 实墨）；
- **减号一点即抹**（底稿还不是事实，不需要长按保险；正式行仍是长按 400ms）；
- **红线：底稿永远不会静默变成数据。** 结束训练时 ghost 行整行丢弃、
  只剩底稿的动作连动作一起丢、全场无描实则拒绝结束并提示。

不继承：力竭（当日事实）、递减子组（结构性，长按重建）。继承：instanceConfig。
组号长按在底稿行上＝先描实再加子组（update 先行，语义自然成立）。
读取路径必须把 ghost 当不存在：刊头组数与容量、时间线组数与胶囊、
bestLifts、热力图 —— 未收尾的草稿会带着 ghost 入库（工作台自动持久化），漏一处就是假数据。
旧行为（把上次末组直接写成真实首组）已删除：那是替用户上报他没做的事。

### 12.7 动作卡拖动排序

长按刊头行 500ms（全站长按语言：120ms 静默 + 进度线 + 「拖动排序」标签）
→ 稿纸离桌（shadow-overlay + scale 1.015）→ 拖动中兄弟卡实时让位
（越过中心移一格，每次 8ms 触感）→ 靠近视口上下缘 80px 自动滚页 → 松手滑进槽位。
实现在 `useCardReorder.ts`，仅两张卡以上时挂载。

- ⚠️ 热区保持 `touch-action: pan-y`：长按满之前手指移动 >10px 就取消手势、
  把滚动还给浏览器（commit 3df41f6 的坑）；**抬起之后**才用非被动 touchmove 拦滚动。
- 刊头里的 ⋯ 菜单：pointerdown 命中 button 直接不参与拖动。
- 落定后「先改数据再清位移」：React 18 同批次里 DOM 重排与清 transform
  在同一次绘制前生效，无闪跳。
- 刊头行的长按语义从此归拖动排序；将来要加别的次级入口另找位置。

### 12.8 时间线长按菜单

历史时间线的训练卡：**点按＝展开详情（原语义不动），长按＝操作菜单**
（编辑这次训练 / 补记动作 / 删除·可撤销）。原先每张卡常驻一排
「编辑·补加·删除」按钮 —— 低频操作永久占版面，全部收进长按，卡面只剩内容。
菜单形态与 ExerciseCard 的 ⋯ 菜单同一 idiom（卡内 absolute + 点外关闭），
不另发明浮层体系。

删除走通则 3：`handleDeleteWorkout(id, { skipConfirm: true })` + 既有 toastUndo。
长按达成后松手带出的 click 用 suppressClickRef 吞掉，否则会顺手展开卡片。

### 12.9 翻月横滑

计划页月历的日期区左右拖，1:1 跟手翻月。这是全 App 唯一真正有左右空间关系的
界面（§5.3 特意规定 Tab 之间**不做**横滑，因为没有空间关系 —— 月历恰恰相反，
「翻页」还是这套语言里最正统的隐喻）。‹ › 按钮保留，走同一段落定动画。

- 松手判定复用 §5.3 弹层的速度投影：`|dx| > W×0.28 || |v| > 0.6 px/ms`，
  收尾时长 `clamp(剩余距离/max(|v|,.8), 120, 260)` —— 甩得越快收得越干脆；
- 接管条件：横向 ≥12px 且横>竖（§12.3 的 12px 规则），明显竖滑直接撒手；
- ⚠️ 速度的 dt 钳到 ≥1（§12.3：同毫秒两次 move 会把最快读成 0）；
- ⚠️ 落定回调必须幂等：transitionend 和兜底 timer 双触发会一次翻过两个月（真踩过）；
- 轨道三格（前月/本月/后月），落定后先无过渡归位再换月重渲染。
