...
> 这是 Fit Tracker 视觉重设计的唯一事实来源。实现时照此文档写，与本文冲突的旧代码以本文为准。
>
> 配套可交互样机（真实渲染，含慢放对比）：
...
- ~~卡片组区加稿纸横纹：`repeating-linear-gradient(to bottom, transparent 0 51px, var(--divider) 51px 52px)`。~~
  **2026-08-29 撤销。** 横纹是第二个「线画在哪」的事实来源，只在每行都恰好 52px 时才与
  行自己的 `border-bottom` 重合。递减子组 44px、「＋再加一档」44px —— 开一档子组之后，
  下面所有行都与横纹错开，横纹从行中间穿过去（真机可见）。而组区容器高度恰等于所有行之和、
  下面没有空白，所以横纹在同步时纯属重复、失同步时就是错的，没有它是对的那一档。
  账本横线的观感由每行的 `border-bottom` 独自承担。
  若日后要恢复「空行也有线」：给容器 `min-height`，横纹画在最后一行**之下**的剩余空间
  （伪元素），不要铺在行的身上。
...
印章是这套语言里的符号，不该因为界面语言变了就消失 —— 但也不能原样留着汉字，
那对英文用户是纯粹的装饰。**换的是刻什么，不是怎么刻**：
朱文／白文之别、尺寸、姿态、频率、落章动画，两种语言完全一致。

| | 中文 | 英文 |
|---|---|---|
| 部位印 | 胸 肩 背 腿 臂 制 · Ma Shan Zheng 30px | **C S B L A O** · Noto Serif SC 700 36px |
| PR 落章 | 破 · Ma Shan Zheng 26px | **PR** · Noto Serif SC 700 30px |

**英文为什么不共用 Ma Shan Zheng。** 它有全套拉丁字形（cmap 查过，且子集里本来就切了
可打印 ASCII，不需要动 `SEAL_CHARS`），但拉丁不是这支笔的主场：52px 方印内宽 48，
「胸」的墨占 30 宽，同尺下手写体的 C 只有 13.1 —— 不到三成，方框里明显显空，
46px 扇开时更弱。换成刊头同族的衬线 700 后 C 占 25.3，密度回到汉字量级。
而且**方框＋首字母在拉丁世界本来就是藏书票／活字首字的语汇**，
不必借道汉字才成立 —— 这和「印章赢在它是这套语言里已有的符号」是同一条理由，
只是换到了另一套语言里。

落章取 **PR** 而不是 NEW／BEST：与 App 里已有的「PR Hub」「By PR」同词，
健身语境下不用解释；两个字母在 64px 方框里也最饱满（NEW 顶边，BEST 得压到 19px）。

⚠️ **大写字母不能沿用汉字那点上内边距。** Ma Shan Zheng 字面偏上要补 1px 压回来；
大写字母全在基线以上、本来就偏低，再加就更歪。实测 52px 印内高 48、C 的墨高 29：
`padding-top: 0` 时上 10 / 下 9，加 2px 就成了上 12 / 下 7。扇开的 46px 同理清零。

三个候选（手写体单字母／衬线单字母／等宽三字母）的并排对比、以及四枚候选落章的
重播，都在 `docs/demos/seal-latin.html`（**要用 dev server 开**，file:// 下取不到字体）。

⚠️ **英文印面的字形保证比中文弱一档，靠 `fontGuard` 补。**
中文印面是 Ma Shan Zheng（`font-display: block` + 预加载），一定是对的字形；
英文印面搭在正文那张衬线上，而 §3 给它选的是 `optional` —— 错过窗口就落成系统衬线，
「印章的全部意义就是那个字形」这条在英文侧就破了。
不能给衬线改 `block`：它同时是全站刊头，一改就是每次冷启动都阻塞。
所以 `src/utils/fontGuard.ts` 的探针从只探 Noto Sans SC 扩成同时探 Noto Serif SC，
任一张没上 DOM 就走那次一次性重载。实测三张正文字体是一起赢一起输的，
...
**英文补签（2026-08-28 补）。** 汉字印面本身就是那个词，扇开即可读；
英文印面是首字母（§12.1），**扇开的一刻六枚印等于六个谜面** ——
页内印谱每枚下面有名字，扇里没有，这个差别只在英文下才变成问题。
所以补签只给英文，中文那版印谱保持干净，不为英文的毛病返工。

| | 位置 | 形制 |
|---|---|---|
| 补签（仅英文） | 印的**径向外侧** R+52 | 无边框无投影的纸片，11px，次级色；悬停转朱砂 |
| 悬停小签（仅中文） | 印的正上方 | 原样不动（有边框有投影，12px） |

两者不并存：英文六张签一直在，再浮一张就是同一个词印两遍。

⚠️ **半径 +52 是两头卡出来的，别随手改。**
下界——印是 46×46 的方块，沿射线方向的半径是 `23/max(|cosθ|,|sinθ|)`，
在 44° 上是 **32.0 而不是 23**（方印的角比边远），加签半高 9.4 与 6px 空隙 = 47.4；
实测 +42 时 Legs / Arms / Other 三张签压在自己那枚印的角上。
上界——最外那张的右边缘要留在屏内，390 宽下 +52 时到 295。

⚠️ **每张签的宽度必须恒定。** 相邻印夹角 17°，在 R+52 的弧上间距 67.3px，
最长的 Shoulders 连内边距 68px 也排得开（它俩是唯一并排的一对，往下就错成上下排了）——
这套间距全建立在「宽度不变」上。试过悬停时把 Other 换成「Other · name it」，
签是居中定位的，一变长就朝两边撑，右下角那枚正好被自己的签压住。
自定名这件事页内印谱底下已经写了，扇里是熟手路径，不必再教一遍。
...
读取路径必须把 ghost 当不存在：刊头组数与容量、**底部常驻栏的「N 组」**、
时间线组数与胶囊、bestLifts、热力图 —— 未收尾的草稿会带着 ghost 入库
（工作台自动持久化），漏一处就是假数据。刊头与底栏口径不同＝其中一个在说假话。
旧行为（把上次末组直接写成真实首组）已删除：那是替用户上报他没做的事。

**进度：把已有状态念出来，不发明新手势。**
底稿一次铺满四行之后，旧的进度信号没了 —— 以前一行一行加，行数就是进度条。
但进度一直在数据里：ghost = 还没做，实墨 = 已做，点组号描实 = 「这组做完了」。
缺的从来不是机制，是没人把它读出来。所以三样都只改读法：

- **分母**：刊头 `1/3组`，分子是已描实。realSetCount 本来就在，没有分母时它读起来
  像「这个动作有几组」；补上分母，同一个数字才变成进度。底稿描完后分母自然消失。
- **落印**：描实后组号从「未刻的虚线印」收成实心朱砂印（`seal-cut`，与 `ink-text` 同拍）。
  只在工作台（`!readOnly`）落 —— 历史里所有组都已入册，印不区分任何东西。
  ⚠ 已知代价：朱砂是 §5.3 签名时刻的稀缺色，每组都盖会稀释它；
  没有底稿的动作卡（全新动作）会整列都是印，那时它同样不区分任何东西。
- **指针**：~~`.is-next` 落在本动作第一条还没描实的行，左缘一道 2.5px 朱砂书签。~~
  **2026-08-29 由 §12.13 休息标记接管**，落点从「行」改成「缝」。
  当年这里担心的「子组会把行撑高、按实测位置平移要挂 ResizeObserver」已被绕开：
  书签是流里的一个零高节点，不测量。

**误触：描实必须可逆（`SetLog.fromGhost`）。**
描实原先是单向的：`ghost:false` 全站只写一次，没有任何地方写回 true。
而误触入口有三个，全都一击成事实 —— 点组号（唯一「零输入就产生数据」的路径）、
点竭（竭自己是 toggle，取消竭却退不回，可逆动作留下不可逆副作用）、横拖越过 12px。
代价还不对称：删一组要长按 400ms 保险（§6.6），凭空造一条没做过的记录却零保险；
误描实的行结束训练时不再被丢弃，它会进 PR / bestLifts / 图表 / 容量，
而删掉之后底稿拿不回来（全站没有「新增一条底稿行」）。这是本节红线上的洞：
不静默，但一击且不可撤销 —— §12.5 通则 3 要的是「先执行 + 撤销」。

解法是让**犯错的手势自己就是反悔的手势**，不走 toastUndo（描实每组一次，
是高频动作，每次弹 toast 会把通则 3 用成噪音）；组号点开点关本来就跟同一行里的
「竭」是一个模式，是对齐不是加法。

- 再点一次组号 = 退回底稿；取消竭 = 若这行除了竭没别的编辑，一并退回。
- 凭据 `fromGhost` 只在「一击描实、且此后没改过值」时存在。它还在就说明行里的值
  仍是底稿原值 —— **退回是无损的，不必另存快照**。
- 任何值编辑清掉它（`VALUE_KEYS`，`toFailure` 故意不在内）；竭亮着时也不给退。
  改过的行退回等于丢弃真实输入，那才是真破坏，仍旧走长按删除。
  **误触产生的恰好就是零编辑的行**，这个限定不多不少正好盖住它。
- ⚠ 三处必须一起剥 `fromGhost`，漏一处就跑偏：结束训练收尾（别跟进历史）、
  「添加组」克隆末行（新行没有底稿可退，留着会被一击退回成底稿）、`toGhostSets`
  （白名单式抄写，天然不带）。
- 眉批把它说出来：「点组号照抄（再点退回）」。撤销可以比主动作难发现，但不能没人提过。
- 一次误退回也不会静默丢数据：眉批回来了、刊头分母回来了、书签跳回去了 ——
  上面那三样进度读数同时就是「你有一行没描实」的警报。
...
同一个动作在不同健身房的配重刻度不一样（龙门架、绳索、固定器械尤其），
换馆会在趋势图上留下一道读不懂的断崖。旧办法是往 `exerciseNotes` 里手写
「fit4less 的重量是这里记录的乘 0.5」——那张备注表实际上已经变成一份
非结构化的器械标定表（单边重量 ×2、杆子 5kg、重量单算片、省力端……）。

**只做标注，不参与计算。** `WorkoutSession.gym` 存字面量场地名，PR / bestLifts /
e1RM / 图表数值一律不按场地分组：杠铃跨馆一致，而会变的那些动作本来就不看纪录。
把它做成"按场地各算一份 PR"是在为一个低频问题引入常驻复杂度。

- **场地表不落 prefs。** 候选从历史 `distinct(w.gym)` 现算（`src/utils/gyms.ts`）。
  `mergeFitlogPrefs` 是逐 key 枚举的，新增一个 key 要同时改
  `FitlogSyncedPrefs` / read / write / merge 四处，漏一处每次同步都会静默吞掉它 ——
  为一个能从既有数据派生的东西付这个风险面不值。
  `WorkoutSession` 上的新增可选字段则随整对象 LWW 免费同步，无需迁移。
- **默认沿用上一场。** 连着去同一个馆永远不用碰那个控件，换馆点一下。
  历史记录不回填、不按日期推断 —— 推断出来的场地是编的。没标就是不知道。
- **图表：信息在「边」上，不在「点」上。** 断崖只发生在跨场地的那一段，
  所以只把那一段连线画成虚线（`3 4` / 0.42），**不给客场的数据点做任何记号** ——
  同一个馆的两个点互相完全可比，标它们是在重复说一件只发生在一处的事。
  两端都标了场地且不同才算跨场地；有一端没标就是不知道，不知道就不画。
  于是这个功能对补标之前的历史零视觉影响。
- ⚠️ Recharts 的 Line 只能整条线一个 `strokeDasharray`。做法是把序列拆成互补的
  派生键：`solid0/solid1` 交替承载同场地的连续块，`cross0/cross1` 交替承载跨场地段
  （`splitByGym`）。**交替是必须的** —— 同一个键里两个相邻的非空值会被
  `connectNulls={false}` 连起来，块与块之间必须隔着一个 null。
  点、末点读数、activeDot 仍挂在完整的 `val` 序列上（`stroke="none"`）。
- **具体在哪个馆练的，点数据点问 tooltip。** 图面上安静，一次交互给答案 ——
  而且用的是图表本来就有的那个交互，不发明新手势（§12.5 通则 1）。
- **底稿眉批带出处（§12.6）。** 上次是别的馆时写成「底稿 · 上次 8月27日 · F4L ——
  刻度可能不同」。**不换算、不过滤**，只是把出处摊开：底稿本来就不是事实。
  ⚠️ `prefillGym` 无条件记来源场地，「跟本场是不是同一个馆」留到渲染时再判 ——
  加完动作再回头改本场场地是常见操作，在预填时定死会留下一条自相矛盾的眉批。
  同 `prefillFrom`，结束训练时剥掉。

### 12.12 时间线密度

一屏只放得下 4 场，而右半张卡是空的。病根不是卡片太胖，是**同一件事说了两遍**：
「天」粒度下一个组就是一天，而一天几乎总是只有一场 —— 于是组头和卡片
必然把日期和计数各说一遍。右侧的空白是同一个病根的另一面：三行信息全部
左对齐堆着，没有任何一列是右对齐的。

**一行三列：日期柱 / 标题+副行 / 计数。** 每场 158px → 72px，一屏 4 场 → 9 场。

- **天粒度且组内只有一场时，组头整个吃掉**，日期改由卡片最左一列承担
  （日号 mono 19px、星期 9.5px），月份降级成一条极轻的标题。
  周/月/年粒度，以及一天里练了两场的情况，仍然走原来的组头 —— 那些场合下
  组头是真的在说新东西。
- **副行换成「时间 · 前三个动作」。** 原先那行是「N 动作 M 组」两个数字，
  而回看时想知道的是「那天练了啥」，不是「练了几个」。数字挪到右列之后，
  这一行不多占高度，白捡的信息。
- 计数右对齐成一列，把右侧空白用掉；展开箭头缩到 16px 跟在计数右边。
- ⚠️ **场地 chip 必须可压缩并自己截断**（`min-w-0 max-w-[5rem]` + 内层 truncate）。
  写死 `flex-none` 的话，一个长场地名会把标题挤到只剩几个字 ——
  标题才是主信息，该让路的是场地。
- 已否决 D 案（去掉卡片外框、整月一张稿纸的账本行，每场 55px）：
  比 C 只多省 17px，代价是每场的存在感弱到长按菜单、展开详情都没有落点。
  §12.8 的长按菜单是卡内 absolute 浮出的，它需要一张卡。
...
### 12.13 休息标记（2026-08-29 实现）

`.is-next` 铺在**行**上，读作「下一组做哪个」。但真正会去看它的时刻是**组间休息**，
那时问的是「我这次休息在哪两组中间」—— 落点在缝上，不在行上。
顺带这么改让隐喻更正确：书签本来就是夹在两页之间的东西，铺满一整行的那个是荧光笔。

- **一半的需求不需要手势。** 「我刚做完第 k 组正在休息」和「第一条没描实的是第 k+1 组」
  是同一个事实的两种说法。所以光把标记挪到缝上，语义就已经对了。
  手势解决的是另一件事：**自动位置不对时把书签挪走**（超级组、跳着做、回头补）——
  这件事此前完全没有入口，不属于 §12.10 说的「省一次点击」。
- **作用域＝全场唯一。** 你同一时刻只可能在一个地方休息。这与旧的每卡各算各的
  `firstGhostIdx` 是根本区别：复制来的训练过去会同时亮好几道书签。
  Q&A：若选「每卡一个」，手势就没有意义 —— 那时它答的还是「这个动作的下一组」。
- **递减档之间不算缝。** 递减组的定义就是**不休息**地往下掉档。所以一组的缝只取
  整块（母行 + 全部递减档 + 「再加一档」）的下边缘 —— 递减做完才轮到休息。
- **手势＝竖向拖丝带**，吸附到最近的缝，落点在松手前先亮（候选缝 + 动作名标签）。
  横滑行不可行：行上的横向手势已被 scrub 占（`.ledger-field.is-scrubbable`），
  行内剩下的自由区只有组号 36 / 竭 36 / 删组 44，全都有语义。
  拖动中丝带脱离卡片改用 `position:fixed` —— 超级组就是在两个动作之间来回，跨不过去
  这个作用域就是假的。
- ⚠️ **静止时零测量**：`RestBookmark` 是 `.ledger-paper` 正常流里的零高节点，
  夹在两个组块之间，CSS 自己把它贴在缝上。递减子组把行撑高多少，缝就跟着走多少。
  只有 pointerdown 那一帧扫一次全场缝位（`scanSeams`，靠 `data-set-idx`）。
- **推动它的三件事**：描实一组 → 跟到那组下面那条缝；**加了新动作 → 跳到新动作最上方**
  （认「id 集合里冒出了没见过的」，不认某一条添加路径）；拖动 → 落到哪算哪。
  描实会覆盖手动位置 —— 描实是关于「我人在哪」最强的证据，拖动管的是两次描实**之间**。
- **位置不是数据**：跟滚动位置同级的「此刻」UI 状态，不进 `SetLog`、不落盘、
  离开工作台即弃。换一场训练整个清空。
- demo：`docs/demos/rest-marker.html`（三个开关 + 六个几何参数，几何值就是从那儿定的）。
